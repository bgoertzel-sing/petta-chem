# Neutral calibration graph-1003 launch review

Status: materialized and launched; raw outcomes embargoed

Date: 2026-08-11

Plan version: `neutral-calibration-v2-g1003-attempt-001`

## Reviewed boundary

This action materialized only the frozen outcome-blind query shard for
`graph_seed=1003` and launched one isolated v2 attempt. It did not open any
...
It declares `prepared-unrun`, `endpoint_results_present=false`, 276
contiguous rows, and 276 unique frozen row keys. Every query imports only the
tracked neutral SSA source and calls `neutral-ssa-run-calibration-row` once.

## Identity review

The launch recomputed and matched the graph-1001/1002 reviewed identities:
...
The focused query-preparation gate, complete-batch preparation gate,
durable-v2-runner synthetic gate, and nine independent RAF-oracle fixtures
passed before launch.

## Attempt and stop rule

Attempt `neutral-v2-g1003-a001` runs in tmux session
`petta-neutral-v2-g1003-a001`, using
`experiments/neutral_calibration_v2/graph-seed-1003/attempt-001`. Initial
metadata inspection found the v2 runner and PeTTa child live, a valid
`ATTEMPT.json`, zero receipts, and only row zero's unreceipted raw pair.
Neither a failure nor a completion manifest existed.

Monitor metadata only. Preserve the attempt at
`raw-complete-unanalysed`; do not inspect any endpoint until every graph
seed 1001--1032 is receipt-complete and a separate real-adapter authorization
