...
`experiments/exp02/smoke.metta` runs the first parameterized deterministic random-polymer control sweep in PeTTa: seed-7/four-rule, seed-11/six-rule, and component-generated seed-13/eight-rule sets are compared with shuffled-catalyst and no-catalysis controls, all rule pairs are scanned by the conservative ACS detector, and each run is represented as a run-contract record.
