...
Passing this smoke means the repo has a working PeTTa runtime path, first atom accessors, event construction, replay equality, a bounded generic binary-catalytic state transformer, candidate atoms, chamber-rule-based candidate generation, candidate-pool/candidate-cap atoms, chamber atoms, metric atoms, applicability checks, nonnegative abundance checks, and a seed/tick deterministic candidate selector wired directly into generated chamber ticking. The generic `candidate-selection-from-pool` seam records the exact bounded list and deterministic selected candidate. `selection-fire-record-from-pool` keeps that full selection provenance beside the chamber produced by firing the exact choice, and generic chamber ticking projects its result from this combined object; `selected-candidate-from-pool` remains a selection-only projection. Candidate generation is evaluated once per bounded pool and is limited to a stable first-eight prefix when a chamber exposes nine through twelve source rules. Chamber firing validates that an external/scored candidate preserves the source rule's stable ID, reactants, and product while allowing catalyst reassignment; rejected choices remain inspectable in selection provenance. All cap-specific candidate-pool and generated-tick compatibility operations are aliases of the generic provenance-backed path, so capping, deterministic selection, and firing have one implementation. Caps are maxima, so a requested cap of eight can safely preserve and select from a smaller three-candidate pool. Complete eight-step live histories derive a PeTTa-native loop action, and bounded two-, three-, and four-sweep drivers consume that action to advance productive sweeps from their exact projected chambers or stop a quiescent sweep without regeneration. Run-wide PeTTa accessors retain total, productive, no-op, and unaccounted outcome counts across all retained sweeps, so a productive run that terminates quiescent remains distinguishable from a run that was quiescent throughout. If chemistry remains productive through a driver's final allowed sweep, it returns `stop-bounded` rather than an unactionable `continue`; the four-sweep fixture retains all thirty-two checked outcomes and the exact final chamber. It does **not** claim ACS emergence.
