...
The stochastic-pilot gate calibrates a replayable four-bin categorical selector over seeds 7--10 and ticks 3--6, then audits the old exp05 chambers before any stateful ensemble is run. The sampler realizes its declared first-candidate masses exactly, but the matched-fixture gate fails: productive first ticks are 0/4 unguided, 3/4 weak Doob-h, and 3/4 shuffled guidance. This is a fixture confound, not a guidance outcome. The exp05 chambers contain only the reactants and catalyst needed by their original deterministic choice, so the next gate is a matched multi-molecule chamber applicability seam that can support every candidate reachable in every arm.
